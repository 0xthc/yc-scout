...
def _build_founder(conn, row) -> dict:
    ...
    return {
        ...
        "sources": sources,
        "tags": tags,
        "score": score_row["composite"] if score_row else 0,
        "scoreBreakdown": {
            "momentum": score_row["momentum"] if score_row else 0,
            "domain": score_row["domain_score"] if score_row else 0,
            "team": score_row["team"] if score_row else 0,
            "traction": score_row["traction"] if score_row else 0,
            "ycfit": score_row["ycfit"] if score_row else 0,
        },
        "signals": signals,
